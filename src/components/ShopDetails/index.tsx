  ...
  useEffect(() => {
    const fetchProduct = async () => {
      ...
      if (!productId || !productType) {
        toast.error('Product ID or type missing');
        setIsLoading(false);
        return;
      }
      ...
      try {
        ...
        const foundProduct = allProducts.find(
          p => p.id === parseInt(productId) && p.type === productType
        );
        ...
        // Get primary image
        const primaryFile =
          foundProduct.yachtPrimaryFile ||
          foundProduct.accessoryPrimaryFile ||
          foundProduct.sparePartPrimaryFile ||
          foundProduct.servicePrimaryFile;

        let primaryImageUrl = primaryFile?.url
          ? `${CONTENT_BASE_URL}${primaryFile.url}`
        ...
        const mappedProduct = {
          ...
          imgs: {
            thumbnails: foundProduct.images && foundProduct.images.length > 0 ? foundProduct.images : [primaryImageUrl],
            previews: foundProduct.images && foundProduct.images.length > 0 ? foundProduct.images : [primaryImageUrl],
          },
        };
