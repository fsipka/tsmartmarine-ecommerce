...
import SparePartCategoryDropdown from "./SparePartCategoryDropdown";

const ShopSparePartSidebar = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        // Filter only spare part products
        const spareParts = products.filter(p => p.type === 'spare-part');

        console.log('Spare part products:', spareParts);
        if (spareParts.length > 0) {
          console.log('First spare part:', spareParts[0]);
        }

        setAllProducts(spareParts);
        setFilteredProducts(spareParts);
      } catch (error) {
  ...
  useEffect(() => {
    ...
    if (brandId) {
      const numericId = parseInt(brandId);
      if (!isNaN(numericId)) {
        setSelectedFilters(prev => ({
          ...prev,
          categories: [],
  ...
  useEffect(() => {
    ...
    if (selectedFilters.brands.length > 0) {
      const numericBrands = selectedFilters.brands
        .filter(id => typeof id === 'number') as number[];
  ...
  const handleCategorySelect = (categoryId: number | string) => {
    ...
    setSelectedFilters(prev => ({
      ...
      brands: [], // Clear brand filters when selecting category
  ...
  const handleSubCategorySelect = (subCategoryId: number | string) => {
    ...
    setSelectedFilters(prev => ({
      ...
      brands: [], // Clear brand filters when selecting subcategory
  ...
  const handleBrandSelect = (brandId: number | string) => {
    const isAllOption = typeof brandId === 'string' && brandId.startsWith('all-');

    setSelectedFilters(prev => ({
      ...prev,
      categories: [], // Clear category filters when selecting brand
      subCategories: [], // Clear subcategory filters when selecting brand
      brands: isAllOption
        ? prev.brands.includes(brandId)
          ? prev.brands.filter(id => id !== brandId)
          : [...prev.brands.filter(id => typeof id === 'number'), brandId]
        : prev.brands.includes(brandId)
          ? prev.brands.filter(id => id !== brandId)
          : [...prev.brands.filter(id => !String(id).startsWith('all-')), brandId],
    }));
  };
  ...
  return (
    <>
      <Breadcrumb title="Spare Parts" pages={["home", "shop"]} />
            ...
            <div className="flex-1">
              ...
              {loading ? (
                ...
                <div className="bg-white rounded-[10px] shadow-1 py-20 text-center">
                  <p className="text-gray-4 text-lg">No spare parts found</p>
                </div>
              ) : (
                <div
                  className={
...
export default ShopSparePartSidebar;
